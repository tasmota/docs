...
Enpoint `root` (0) is managed by the `matter.Plugin_Root` class because of its specific behavior.
...
sessions|instance of `matter.Session_Store()` which holdslist of `matter.Session()`<BR>All active persistant and non-persistant sessions are listed here, and serve to dispatch incomming packets<BR>Session are also linked to `Fabric` when persisted
...
compute_manual_pairing_code()|Compute the 11 digits manual pairing code (wihout vendorid nor productid) p.223<BR>can be done only for root PASE (we need the passcode, but we don't get it with OpenCommissioningWindow command)
...
get_active_endpoints(exclude_zero)|Return the list of endpoints from all plugins (distinct), exclud endpoint zero if `exclude_zero` is `true`
save_param()|Persistance of Matter Device parameters
...
Method|Description
:----|:---
start_mdns_announce_hostnames()|Start mDNS and announce hostnames for Wifi and ETH from MAC<BR>When the announce is active, `hostname_wifi` and `hostname_eth` are defined
...
\_resend_packets()|Resend packets if they have not been acknowledged by receiver either with direct Ack packet or ack embedded in another packet. Packets with `id`=`nil` are not resent.<BR>Packets are re-sent at most `RETRIES` (4) times, i.e. sent maximum 5 times. Exponential backoff is added after each resending.<BR>If all retries expired, remove packet and log.
...
Method|Description
:----|:---
init(device)|Constructor, instanciates monads for `commissioning` and `im`
...
send_response(raw, addr, port, id, session_id)|Send a packet. Proxy to the same mehtod in `device`
...
Method|Description
:----|:---
init(im)|Instanciate the monad with the global IM monad
new\_subscription(session, req)|Take a session and a `SubscribeRequestMessage`, parse the message and create a `matter.IM_Subscription` instance.<BR>Returns the `matter.IM_Subscription()` instance. Also allocates a new subcription id.

## What happens when an attribute is updated

Subscriptions are triggered by the value of an attribute changing. They can originate from an explicit WRITE Matter command from an initiator or another device, of be the consequence of a Matter command (like switching a light ON). The can also originated from independant source, like an action at Tasmota level (using Tasmota `Power` command), or Tasmota detecting that a sensor value has changed after periodical polling.
...
Calls to `attribute_updated_ctx()` are first check whether the attribute matches the filtering criterias (that may include wildcards). If they match, the attribute is candidate to be added to the list. We then call `_add_attribute_unique_path()` to check if the attribute is not already in the list, and if not add it to the list of future updates. It's possible that during the `min_interval` time, an attribute may change value multiple times; yet we publish a single value (the last one).
