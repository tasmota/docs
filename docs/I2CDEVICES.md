...
Historically (ESP82xx) Tasmota has only supported a single I2C interface, however with ESP32 two hardware I2C interfaces are not available can be configured. This has led to the unfortunate situation that, while you can *configure* two I2C interface in Tasmota most I2C dirvers only support using the first bus.
...
The *Bus 2* column lists if the given driver supports devices connected to a bus other than bus 1:
  * ✅: Both bus 1 and bus 2 are supported at the same time (allowing two peripherals sharing the smae address)
  * ☑️: Both bus 1 and bus 2 are supported, but not at the same time (allowing only one peripheral)
  * ❓: Status is currently undocumented – most likely only bus 1 is supported –
    * If possible please check the driver source code and update the table afterwards!
  * ❌: Only bus 1 is supported

Index | Define              | Driver   | Device   | Address(es) | Bus 2 | Description
------|---------------------|----------|----------|-------------|-------|-----------------------------------------------
  ...
  58  | USE_MPU_ACCEL       | xsns_85  | MPU_ACCEL| 0x68        | ☑️    | MPU6886/MPU9250 6-axis MotionTracking sensor from M5Stack
