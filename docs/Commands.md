...
Sensor83<a class="cmnd" id="sensor83"></a>|Sugar Valley NeoPool Controller<BR>`1 <addr> {<cnt>}` = read `cnt` (`1..30`) 16-bit register starting from `addr` - cnt=`1` if omitted<BR>`2 <addr> <data> {<data>...}` = write 16-bit register starting from `addr` using `data` (`0..65535|0..0xFFFF`) (max 8 `data` at once)<BR>`3 <addr> <bit> {<data>}` = read register bit (`0..15`) from `addr` if `data` is omitted, otherwise set `bit` of `addr` to `data` (`0|1`)<BR>`4 {<state>}` = get filtration state if `state` is omitted, otherwise set `state` to `0|1`<BR>`5 {<mode>}` = get filtration mode if `mode` is omitted, otherwise set `mode` (`0..4|13`)<BR>`6 {<time>}` = get current device time if `time` is omitted, otherwise set device to new time: `0` sync device time to Tasmota local time, `1` sync device time to Tasmota UTC time, any other value set device time as epoch<BR>`16 <addr> {<cnt>}` = same as `1` but using hex data output<BR>`21 <addr> {<cnt>}` = same as `1` but reading 32-bit register, `cnt`=`1..15`<BR>`22 <addr> <data> {<data>...}` = same as `2` but writing 32-bit register, `cnt` = `1..15`, `data` = `0..0xFFFFFFFF`<BR>`26 <addr> {<cnt>}` = same as `21` but using hex data output
