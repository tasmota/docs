...
WebQuery<x\><a class="cmnd" id="webquery"></a>|Command for GET, POST, PUT, and PATCH HTTP queries, complete with Request Headers and request body (when applicable). <BR> Note: If a response is needed you must compile your own bin with `#define USE_WEBSEND_RESPONSE`. If the site uses `https:`this (and many other functions) will only work with ESP32s as ESP8266 will not work with `https:`<BR> `<url> GET|POST|PUT|PATCH [<headers>] <body>`<BR>[More information...](https://github.com/arendst/Tasmota/pull/13209)
