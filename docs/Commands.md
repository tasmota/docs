...
Sensor83<a id="sensor83"></a>|`1 <addr> {<cnt>}` = read 16-bit register (cnt = `1..30`). cnt=1 if omitted.<BR>`2 <addr> <data> {<data>...}` = write 16-bit register (data = `0..65535|0x0000..0xFFFF`). Use `<data>` max 8 times.<BR>`3 <addr> <bit> {<data>}` = read/write register bit (bit = `0..15`, data = `0|1`). Read if `data` is omitted, otherwise write.<BR>`4 {<state>}` = get/set  filtration state (state = `0|1`). Get if `state` is omitted, otherwise set new state.<BR>`5 {<mode>}` = get/set filtration mode (mode = `0..4|13`). Get if `mode` is omitted, otherwise set.<BR>`6 {<time>}` = get/set device time (time = `0..4294967295|0..0xFFFFFFFF`). Get if `time` is omitted, otherwise set accordingly:<BR><ul><li>`0` - sync device time with Tasmota local time</li><li>`1` - sync device time with Tasmota utc time</li><li>`2..4294967295`- set device time as epoch</li></ul><BR>`16 <addr> {<cnt>}` = same as read 16-bit register (Sensor83 1) but using hex data output.<BR>`21 <addr> {<cnt>}` = read 32-bit register (cnt = `1..15`), cnt=1 if omitted.<BR>`22 <addr> <data> {<data>...}` = write 32-bit register (data = `0..4294967295|0..0xFFFFFFFF`). Use `data` max 8 times.<BR>`26 <addr> (<cnt>}` = same as read 32-bit register (Sensor83 21) but using hex data output.
