...
LedPwmMode<a class="cmnd" id="ledpwm"><x\>|Enable pwm mode for LED<x\>; brightness of LED on and off states can be configured<BR>`0` = disable pwm mode for LED<x\> *(default)* <BR> `1` = enable pwm mode for LED<x\><BR>_Note: PWM mode cannot be enabled for LedLink(i). However, if LedLink(i) is not configured and Led1(i) is, LedState uses LED1 and LedPwm1 setting._
