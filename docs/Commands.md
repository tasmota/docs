...
Sensor20<a id="sensor20"></a>|[Nova Fitness SDS011](SDS011) dust sensor.<BR>1..255` = number of seconds before TelePeriod to poll the sensor
...
Sensor52<a id="sensor52"></a>|iBeacon driver with [HM10](HM-10) or [HM17/HM16](HM-17)<BR>`1` and `2` = required only once to initialize the module  <BR>`u<x>` = sets update interval in seconds (scan tags every \<x\> seconds) _(default = 10)_<BR>`t<x>` = set timeout interval in seconds (send RSSI=0 if tag is not detected after \<x\> seconds) _(default = 30)_<BR>`d1` = enable debug mode (shows all serial traffic in console)<BR>`d0` = disable debug mode_(default = 30)_<BR>`c` = clears iBeacon list<BR>`s AT+<command>`  = send native AT commands 
...
SetOption89<a id="setoption89"></a>|Configure MQTT topic for Zigbee devices (also see `SensorRetain`)<BR>`0` = single `tele/%topic%/SENSOR` topic _(default)_ <BR>`1` = unique device topic based on Zigbee device ShortAddr<br>_Example:_ `tele/Zigbee/5ADF/SENSOR = {"ZbReceived":{"0x5ADF":{"Dimmer":254,"Endpoint":1,"LinkQuality":70}}}`


<a id="serial-bridge"></a>
...
ShutterStop&#60;x><a id="shutterstop"></a>|Disengage the relays to stop the shutter. Number of shutter can be the index or the argument
...
ZbSend<a id="zbsend"></a>|`{"Device":"<shortaddr>", "Endpoint":"<endpoint>", "Manuf":<manuf>, "Send":{"<sendcmd>":<sendparam>}}`<BR>&emsp;`<shortaddr>` the short address of the Zigbee device on the network<BR>&emsp;`<endpoint>` the target endpoint on the device ([understandig endpoints](Zigbee#understanding-endpoints))<BR>&emsp;`<manuf>` (optional) forces a specific ManufacturerId in the ZCL frame (required by some Xiaomi devices)<BR>&emsp;`"<sendcmd>":<sendparam>` the command and parameters to send ([Zigbee Device Commands](Zigbee#supported-zigbee-device-commands))<BR>Note: Use [`ZbZNPSend`](#ZbZNPSend) to send a raw form low-level message<BR><BR>Ex: `ZigbeeSend { "Device":"0x1234", "Endpoint":"0x03", "Send":{"Power":"on"} }`
...
ZbRestore<a id="zbrestore"></a>|Restores a device configuration previously dumped with `ZbStatus2`. This command does not pair a device, but lets you get back device configuration like ModelId or IEEEAddress.<BR>&emsp;`<json>` where json contains the fields dumped with `ZbStatus2`. `<json>` can contain multiple devices (if they fit).
