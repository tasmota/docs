...
LedPwmMode<a class="cmnd" id="ledpwmmode"><x\>|Enable pwm mode for LED<x\>; brightness of LED on and off states can be configured<BR>`0` = disable pwm mode for LED<x\> *(default)* <BR> `1` = enable pwm mode for LED<x\><BR>_Note: PWM mode cannot be enabled for LedLink(i). However, if LedLink(i) is not configured and Led1(i) is, LedState uses LED1 and LedPwm1 setting._
LedPwmOn<a class="cmnd" id="ledpwmon">|Returns PWM value for LED on state when LedPwm is enabled.<BR>`0..255` Set PWM value, between 0 (fully off) and 255 (fully on)
LedPwmOff<a class="cmnd" id="ledpwmoff">|Returns PWM value for LED off state when LedPwm is enabled.<BR>`0..255` Set PWM value, between 0 (fully off) and 255 (fully on)    
