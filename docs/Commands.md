...
Sensor13<a class="cmnd" id="sensor13"></a>|[INA219](http://www.ti.com/product/INA219) and [ISL28022](https://www.renesas.com/us/en/products/analog-products/amplifiers/current-sense-amplifiers/isl28022-precision-digital-power-monitor) low voltage current sensor configuration<BR>**Legacy modes using standard 0.1 ohm resistor for all INA219/ISL28022:**<BR>`0`..`2` = set INA219 calibration to max 32V and 2A<BR>In all cases, ISL28022 is set to 60V mode<BR>**Extended modes for more shunt resistor values using compact mode (all INA chips):**<BR>`10`..`255`: Define custom shunt resistor encoded as a decimal number `RRM` such that `Rshunt = RR * 10^M` milliohm<BR>Examples:<BR>`11` = 1 * 10^1 = 10 milliohm (Imax=32A => Pres=15W)<BR>`21` = 2 * 10^1 = 20 milliohm (Imax=16A => Pres=7W)<BR>`12` = 1 * 10^2 = 100 milliohm (default, Imax=3.2A => Pres=2W)<BR>`13` = 1 * 10^3 = 1000 milliohm = 1 ohm (Imax=0.320A => Pres=0,2W)<BR>**Extended modes for any shunt value, per INA chip:**<BR>`0, <rshunt1>, <rshunt2>, <rshunt3>, <rshunt3>`: Define shunt resistors with floating point in ohms, 1 value per INA chip, up to 4 chips.<BR>Exemple:<BR>`Sensor13 0, 0.1, 0.01, 1.0, 1.0` Defines the shunt resistor for 4 INA chips with respective current range of 3.2A, 32A and 0.320A<BR>**Warning:** This mode is not saved to flash settings and the command must be applied at every boot using a boot rule such as `on system#boot do Sensor13 0, 0.1, 0.01, 1.0, 1.0 endon` or within `autoexec.bat`<BR>**ISL28022:** The driver seamlessly detect ISL28022 vs INA219 and adapt configuration and readings accordingly. The component label in Web GUI and SENSOR message will automatically match the detected part. It is possible to mix INA219 and ISL28022 as far as addresses do not conflicts.<BR>**Warning:** Do not forget to choose a resistor which power rating is adapted for the correct power dissipation. Preferably apply a 50%..100% security margin! Watts in a resistor translate as heat, so for high current, have the resistor well ventilated.
