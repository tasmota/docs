...
DisplayText<br>*(TM1637 only)* | `text`[, `position`[, `length`]] <br>Clears and then displays basic text on the 7-segment display. <br><br> `length` can be `1` to `NUM_DIGITS` , <br> `position` can be `0` (left-most) to `NUM_DIGITS-1` (right-most) <br><br>  A caret(`^`) symbol in the text input is dispayed as the degrees(`°`) symbol. This is useful for displaying Temperature (or angle)! <br>See [TM1637](TM1637.md) for details.
DisplayTextNC<br>*(TM1637 only)*  | `text`[, `position`[, `length`]] <br>Clears first, then displays text. Usage is same as above. <br>See [TM1637](TM1637.md) for details.
...
DisplayType<br>*(TM1637 only)*<a class="cmnd" id="displaytype"></a>|Specify the variant of 7-segment display <br>`0` = TM1637 4-digit<br>`1` = TM1637 6-digit. <br>See [TM1637](TM1637.md) for details.
DisplayClear<br>*(TM1637 only)*<a class="cmnd" id="displayclear"></a>|Clears the display. <br>See [TM1637](TM1637.md) for details.
DisplayNumber <br>*(TM1637 only)*<a class="cmnd" id="displaynumber"></a>| `num` [, `position`[, `leading_zeros`[, `length`]]] <br>Clears and then displays number `num` without decimal.<br><br> `leading zeros` can be `1` or `0` (default),<br> `length` can be `1` to `NUM_DIGITS` (4 or 6), <br> `position` can be `0` (left-most) to `NUM_DIGITS` (right-most). <br>See [TM1637](TM1637.md) for details.
DisplayNumberNC <br>*(TM1637 only)* | `num` [, `position`[, `leading_zeros`[, `length`]]] <br>Display integer number as above, but without clearing first. Usage is same as above. <br>See [TM1637](TM1637.md) for details.
DisplayFloat <br>*(TM1637 only)* | `num`[, `position`[, `precision`[, `length`]]] <br>Clears and then displays float (with decimal point).<br><br> `precision` can be `0` to `NUM_DIGITS` (default), <br> `length` can be `1` to `NUM_DIGITS` (4 or 6), <br> `position` can be `0` (left-most) to `NUM_DIGITS` (right-most). <br>See [TM1637](TM1637.md) for details.
DisplayFloatNC <br>*(TM1637 only)* | `num`[, `position`[, `precision`[, `length`]]] <br>Displays float (with decimal point) as above, but without clearing first. Usage same as above. <br>See [TM1637](TM1637.md) for details.
DisplayBrightness | `1..8` Set brightness of the display. <br><br>See [TM1637](TM1637.md) for details.
DisplayRaw <br>*(TM1637 only)* | `position`, `length`,  `num1` [, `num2`[, `num3`[, `num4`[, ...upto `NUM_DIGITS` numbers]]...] <br>Takes upto `NUM_DIGITS` comma-separated integers (0-255) and displays raw segments. <br>  <br> `length` can be `1` to `NUM_DIGITS` (4 or 6), <br> `position` can be `0` (left-most) to `NUM_DIGITS` (right-most). <br>`num1`, `num2`, ...  are numbers representing a 7-segment digit. Each number represents all segments of one digit. <br> Segment a=1, b=2, c=4, d=8, e=16, f=32, g=64 and h (decimal point)=128.<br> To turn on all segments, the number would be 1+2+4+8+16+32+64+128 = 255. <br>See [TM1637](TM1637.md) for details.
DisplayScrollText<br>*(TM1637 only)* | `text` Displays scrolling text, upto 50 characters. <br>Command example: `DisplayScrollText tasmota is awesome`. <br>See [TM1637](TM1637.md) for details.
DisplayScrollDelay<br>*(TM1637 only)* | `0..15` Sets the speed of text scroll. Smaller delay implies faster scrolling. <br> <br>See [TM1637](TM1637.md) for details.
DisplayLevel<br>*(TM1637 only)* | `0..100` Display a horizontal bar graph. <br><br>See [TM1637](TM1637.md) for details.
DisplayClock<br>*(TM1637 only)* | Displays a clock. <br> `1` = displays a clock in 12-hour format. <br> `2` = displays a clock in 24-hour format. <br> `0` = turns off the clock and clears the display  <br>See [TM1637](TM1637.md) for details.
