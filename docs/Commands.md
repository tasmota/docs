...
IfxPeriod<a class="cmnd" id="ifxperiod"></a>|`<value>`<br>= `0` (default) : use Teleperiod value 
as publication interval<br>= `10`..`3600` : set a different publication interval<br>Note: Even when
`IfxPeriod 0` is used, publication is not necesseraly performed at the time as the telemetry 
message.
