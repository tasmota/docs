...
DisplayTextNC<br>*(TM1637 only)*  | Clears first, then displays text. Usage is same as above. <br>See [TM1637](TM1637.md) for details.
...
DisplayNumberNC <br>*(TM1637 only)* | Display integer number as above, but without clearing first. Usage is same as above. <br>See [TM1637](TM1637.md) for details.
DisplayFloat <br>*(TM1637 only)* | Clears and then displays float (with decimal point).<br>Command parameters: `num`[, `position`[, `precision`[, `length`]]] <br><br> `precision` can be `0` to `NUM_DIGITS` (default), <br> `length` can be `1` to `NUM_DIGITS` (4 or 6), <br> `position` can be `0` (left-most) to `NUM_DIGITS` (right-most). <br>See [TM1637](TM1637.md) for details.
DisplayFloatNC <br>*(TM1637 only)* | Displays float (with decimal point) as above, but without clearing first. Usage same as above. <br>See [TM1637](TM1637.md) for details.
DisplayBrightness | Set brightness of the display. <br>valid values are `1` (lowest) to `8` (highest). <br>See [TM1637](TM1637.md) for details.
...
DisplayScrollText<br>*(TM1637 only)* | Displays scrolling text, upto 50 characters. Command example: `DisplayScrollText tasmota is awesome`. <br>See [TM1637](TM1637.md) for details.
DisplayScrollDelay<br>*(TM1637 only)* | Sets the speed of text scroll. Smaller delay implies faster scrolling. <br> valid values are `0` (highest speed) to `15` (lowest speed). <br>See [TM1637](TM1637.md) for details.
DisplayLevel<br>*(TM1637 only)* | Display a horizontal bar graph. <br>valid values are `0` (level 0) to `100` (level 100%). <br>See [TM1637](TM1637.md) for details.
