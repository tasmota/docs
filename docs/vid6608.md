...
* `VID6608_RESET_ON_INIT` - Enables default homing operation at startup, default is `true` (see notes below);
* `VID6608_RMT` - Enables hardware timer to generate pulse for driver, default is `false` (see notes balow);
...
Driver has option to enable the [ESP-32 Remote Control Transceiver (RMT)](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-reference/peripherals/rmt.html),
this driver can generate pulse for drive much better and provides smooth movement.

??? bug "RMT driver may conflict with other features"

    ESP's have limited number of RMT drivers onboard. This feature may conflict with other
    drivers, i.e. with WS2812b. Use with cauton. If drivers number is exceed on platform,
    you should see error `0x105`:
    ``` c
    ESP_ERROR_CHECK failed: esp_err_t 0x105
    ```
