...
Attributes|Details
:---|:---
Leds<a class="cmnd" id="leds_ctor"></a>|`Leds(pixels:int, gpio:int [,rmt:int]) -> instance<Leds>`<br>Creates a `Leds` instance for a linear leds strip<br>`pixels`: number of leds<br>`gpio`: gpio number<br>`rmt`: (opt) `RMT`channel to use, or auto-select (see below)
Leds\_matrix<a class="cmnd" id="leds_matrix_ctor"></a>|`Leds_matrix(width:int, height:int, gpio:int [,rmt:int]) -> instance<Leds_matrix>`<br>Creates a `Leds` instance for a matrix of leds<br>`width`: number of leds horizontally<br>`hieght`: number of leds vertically<br>`gpio`: gpio number<br>`rmt`: (opt) `RMT`channel to use, or auto-select (see below)
