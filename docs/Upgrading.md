...
Follow the path strictly to ensure success. **Do not install only tasmota-minimal.bin** but upgrade to full, working firmware. Linked `-lite.bin` binaries will do the job, copy the URL behind each step in the flow. If you want to download binaries from the upgrade flow links, you may need to do a right-click and save the file, depending on your browser security rules. 
