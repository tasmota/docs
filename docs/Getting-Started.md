...
**Note: ShellyPlugUS is an exception to the above rule. TX on the board goes to TX on the serial adapter, RX to RX.**
...
* Make sure that the RX pin is connected to the TX pin between the serial adapter and your ESP device, and vice versa. Note: some devices, such as the ShellyPlugUS use TX to TX and RX to RX. When in doubt try swapping TX and RX connections.
