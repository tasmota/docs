...
:---|:---
ShutterButton&#60;x><a id="shutterbutton"></a>|`<a> <b> <c> <d> <e> <f> <g> <h> <i> <j>`<BR>`<x>`: Button number 1..4<BR>`<a>`: Shutter number 1..4<BR>`<b>`: 0..100/up/down/updown, `-` to disable, single press move position<BR>`<c>`: 0..100, `-` to disable, double press move position<BR>`<d>`: 0..100, `-` to disable, tripple press move position<BR>`<e>`: 0..100, `-` to disable, hold press move position<BR>`<f>`: 0/1, disable/enable single press position MQTT command broadcast<BR>`<g>`: 0/1, disable/enable single double position MQTT command broadcast<BR>`<h>`: 0/1, disable/enable single tripple position MQTT command broadcast<BR>`<i>`: 0/1, disable/enable hold position MQTT command broadcast<BR>`<j>`: 0/1, disable/enable MQTT command broadcast all shutter indices<BR><BR>Assign botton `<x>` to control shutter `<a>`. For more details please see also [Blinds and shutters](blinds-and-shutters) support<BR><BR>For example, to control a shutter by two buttons: "`backlog ShutterButton3 1 up; ShutterButton4 1 down`" assigns button 3 to act as an "up" button and button 4 to act as an "down" button for shutter 1.<BR><BR>For example, to control a shutter by one buttons: "`ShutterButton3 1 updown`" assigns button 3 to act as an "up&down" button for shutter 1.
