...
:---|:---
ShutterButton&#60;x><a id="shutterbutton"></a>|`<a> <b> <c> <d> <e> <f> <g> <h> <i> <j>`<BR>`<x>`: Button number 1..4<BR>`<a>`: Shutter number 1..4<BR>`<b>`: 0..100, `-` to disable, single press move position<BR>`<c>`: 0..100, `-` to disable, double press move position<BR>`<d>`: 0..100, `-` to disable, tripple press move position<BR>`<e>`: 0..100, `-` to disable, hold press move position<BR>`<f>`: 0|1, disable|enable single press position MQTT command broadcast to `<grouptopic>`<BR>`<g>`: 0|1, disable|enable single double position MQTT command broadcast to `<grouptopic>`<BR>`<h>`: 0|1, disable|enable single tripple position MQTT command broadcast to `<grouptopic>`<BR>`<i>`: 0|1, disable|enable hold position MQTT command broadcast to `<grouptopic>`<BR>`<j>`: 0|1, disable|enable MQTT command broadcast all shutter indices<BR><BR>Assign botton `<a>` to control shutter `<x>`.<BR><BR>Setup button `<x>` as "up" button for relay `<a>`: `ShutterButton<x> <a> up`.<BR>Single press will move shutter up to 100%, double press to 50% and tripple press to 74%. Holding the button for more than the hold time (SetOption32) moves all shutters with same `<grouptopic>` up to 100%.<BR><BR>Setup button `<x>` as "down" button for relay `<a>`: `ShutterButton<x> <a> down`.<BR>Single press will move shutter down to 0%, double press to 50% and tripple press to 24%. Holding the button for more than the hold time (SetOption32) moves all shutters with same `<grouptopic>` down to 0%.<BR><BR>Setup button `<x>` as "updown" button for relay `<a>`: `ShutterButton<x> <a> updown`.<BR>Single press will move shutter up to 100%, double press down to 0% and tripple press to 50%.
