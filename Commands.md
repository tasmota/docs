...
Sensor52<a id="sensor52"></a>|[HM17](/peripherals/HM-17)<BR>`1` and `2` = required only once to initialize the module  <BR>`u<x>` = sets update interval in seconds (scan tags every \<x\> seconds) _(default = 10)_<BR>`t<x>` = set timeout interval in seconds (send RSSI=0 if tag is not detected after \<x\> seconds) _(default = 30)_
