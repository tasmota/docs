...
:---|:---
ShutterButton&#60;x><a id="shutterbutton"></a>|`<button> <func> <mqtt>`<BR><BR>Assign botton `<button>` to control this shutter. For more details please see also [Blinds and shutters](blinds-and-shutters) support<BR><BR>`<button>` `1..4`: Button number, `0`: disable buttons for this shutter<BR>`<func>` `up`/`down`/`updown`: functionality of that button<BR>`<mqtt>` `1`/`0`: enable/disable MQTT broadcast for hold action of button<BR><BR>`no parameters`: show configuration for this shutter<BR><BR>For example, to control shutter #1 by two buttons #1 and #2: "`backlog ShutterButton1 1 up; ShutterButton1 2 down`" assigns button #1 to act as an "up" button and button #2 to act as an "down" button for shutter #1.<BR><BR>For example, to control shutter #1 by a single button #1: "`ShutterButton1 1 updown`" assigns button #1 to act as an "up and down" button for shutter #1.
